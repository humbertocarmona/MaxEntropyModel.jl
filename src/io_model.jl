...
function set_model!(model::MaxEnt, dic::Dict)
	...
	haskey(dic, :ones_dist_obs) &&
		(model.ones_dist_obs = Vector{Float64}(dic[:ones_dist_obs]))
	...
	haskey(dic, :ones_dist_mod) &&
		(model.ones_dist_mod = Vector{Float64}(dic[:ones_dist_mod]))
