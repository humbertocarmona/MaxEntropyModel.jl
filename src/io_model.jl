function save_model(model::MaxEnt, filename::String="none")
    if filename == "none"
...
function set_model!(input_file::String)
    obj = JSON3.read(input_file)
    @assert haskey(obj, "model") "$input_file must have the \"model\" key"
    @assert haskey(obj, "nspins") "$input_file must have the \"nspins\" key"
    @assert obj.model == "MaxEnt"
