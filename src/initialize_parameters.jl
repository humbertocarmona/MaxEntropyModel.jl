function init_parametes!(m::MaxEnt, h::Vector{Float64}, J::Vector{Float64})
...
function init_parametes!(m::MaxEnt, μh::Float64, σh::Float64, μJ::Float64, σJ::Float64)
    disth = Normal(μh, σh)
    distJ = Normal(μJ, σJ)
...
function init_parametes!(m::MaxEnt)
    m.h = copy(m.x_obs)
    m.J = copy(m.xy_obs)
end
